...
const GlowCard: React.FC<GlowCardProps> = ({ 
  ...
  useEffect(() => {
    ...
    const syncPointer = (e: PointerEvent) => {
      ...
      card.style.setProperty('--xp', (x / card.offsetWidth).toFixed(2));
      card.style.setProperty('--y', y.toFixed(2));
      card.style.setProperty('--yp', (y / card.offsetHeight).toFixed(2));
    };
  ...
  const getInlineStyles = (): React.CSSProperties => {
    const baseStyles: React.CSSProperties = {
      '--base': 280, // Purple
      '--spread': 50, // Purple (280) to Pink (330)
      '--radius': '14',
      '--border': '3',
  ...
  const beforeAfterStyles = `
    ...
    [data-glow]::after {
      ...
      transition: --bg-spot-opacity 0.2s ease, --border-spot-opacity 0.2s ease, --border-light-opacity 0.2s ease;
    }
